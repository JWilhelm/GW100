...
n_molecules_homo='0'
n_molecules_lumo='0'
n_molecules_homo_accurate='0'
n_molecules_lumo_accurate='0'
...
do
  ...
  homo=$(grep "( occ )" $outfile | tail -1 | awk '{ print $8 }')
  lumo=$(grep "( vir )" $outfile | head -1 | awk '{ print $8 }')
  ...
  n_molecules_homo_accurate=$(echo "(($homo-($homo_ref))^2 < $accuracy^2) + $n_molecules_homo_accurate"| bc -l)
  n_molecules_lumo_accurate=$(echo "(($lumo-($lumo_ref))^2 < $accuracy^2) + $n_molecules_lumo_accurate"| bc -l)
...
echo "Number of molecules with HOMOs more accurate than " $accuracy "eV: " $n_molecules_homo_accurate
echo "Number of molecules with LUMOs more accurate than " $accuracy "eV: " $n_molecules_lumo_accurate
